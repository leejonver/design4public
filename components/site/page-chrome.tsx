...
export function Breadcrumb({ items }: { items: Crumb[] }) {
  return (
    <nav style={{ display: "flex", alignItems: "center", gap: 8, fontFamily: "var(--font-sans)", fontSize: "var(--fs-sm)", color: "var(--ink-400)", flexWrap: "wrap" }}>
      {items.map((it, i) => {
    ...
    <section style={{ borderBottom: "1px solid var(--border-hair)" }}>
      <Container style={{ padding: "var(--sp-5) var(--gutter) var(--sp-4)" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 20, marginTop: breadcrumb ? 10 : 0 }}>
          <h1 style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: "var(--fs-h2)", letterSpacing: "0.01em", textTransform: "uppercase", color: "var(--ink-900)", margin: 0 }}>{title}</h1>
          {count != null && (
            <span style={{ fontFamily: "var(--font-sans)", fontSize: "var(--fs-body)", color: "var(--ink-500)", whiteSpace: "nowrap", paddingBottom: 4, flex: "none" }}>
              <strong style={{ color: "var(--ink-900)", fontWeight: 700 }}>{count.toLocaleString()}</strong>개
            </span>
          )}
        </div>
        {lead && <p style={{ fontFamily: "var(--font-sans)", fontSize: "var(--fs-body)", color: "var(--ink-500)", margin: "7px 0 0", maxWidth: "54ch", lineHeight: 1.5 }}>{lead}</p>}
      </Container>
    </section>
  );
}
