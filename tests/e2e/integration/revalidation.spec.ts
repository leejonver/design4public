...
test.describe('통합: 관리자 변경 → 사이트 즉시 반영 (원버그 회귀)', () => {
  ...
  test('제목을 수정하면 사이트 상세가 즉시 갱신된다', async ({ page }) => {
    ...
    // Wait for the edit form to hydrate with the loaded project before typing —
    // otherwise the async data fetch resolves after fill() and overwrites the
    // new title, so the PUT would persist the old one.
    const nameField = page.getByPlaceholder('프로젝트명을 입력하세요')
    ...
    // the PUT completes and the next navigation would abort it.
